...
(1)	Data and plots for final vehicle GHGs were double-checked and revised.  

(2) Leaflets were simplified for effective purpose. 

(3)	For the final plot of all emissions, building + GHG, a bar chart was presented.
...
full_zip_od_routed <- readRDS( "full_zip_od_routed.rds")
...
full_zip_od_final <- readRDS( "full_zip_od_final.rds")
...
Redwood_trips_year <-
  ...
  summarise(
    ...
    SI03=sum(SI03),
    vmt =sum(vmt, na.rm = T),
    vehicle =sum(vehicles, na.rm = T),
    duration=mean(duration, na.rm = T),
    distance=mean(distance, na.rm = T),
    visits=mean(visits, na.rm = T)
)
```

```{r}
Redwood_trips_direction_year <-
  Redwood_trips  %>%
  group_by(direction, year) %>%
  summarise(
    S000=sum(S000),
...
# Redwood_trips_ghg_2019_O <-
#  emfac %>% 
#  mutate(
#    year=2019,
#    trips = Percent_Trips *  sum(Redwood_trips_2019$visits, na.rm = T),
#    vmt = Percent_Miles * sum(Redwood_trips_2019$vmt, na.rm = T),
#    ghg = vmt*MTCO2_Running_Exhaust + trips*MTCO2_Start_Exhaust*2
#  )

# saveRDS(Redwood_trips_ghg_2019_O,"Redwood_trips_ghg_2019_O.rds")
```

```{r}
# Redwood_trips_ghg_full_I <-
#   rbind(
#   Redwood_trips_ghg_2013_I,Redwood_trips_ghg_2014_I,
#    Redwood_trips_ghg_2015_I,Redwood_trips_ghg_2016_I,
#   Redwood_trips_ghg_2017_I,Redwood_trips_ghg_2018_I,
#    Redwood_trips_ghg_2019_I) %>%
#  mutate(
#    direction = "inbound"
#    )

# saveRDS(Redwood_trips_ghg_full_I, "Redwood_trips_ghg_full_I.rds")
```

```{r}
# Redwood_trips_ghg_full <-
#   rbind(
#   Redwood_trips_ghg_full_O,Redwood_trips_ghg_full_I)
...
```{r}
Redwood_trips_ghg_full_temp <- readRDS("Redwood_trips_ghg_full.rds")
```

```{r}
Redwood_trips_ghg_full_temp  %>% 
  filter(direction == "outbound") %>%
  ggplot(
    aes(
      x = year,
  ...
  labs(x = "Year", y = "ghg",
  title = "Outbound GHG of Vehical Emissions in Redwood City, 2013 to 2019", 
...
Redwood_trips_ghg_full_temp  %>%
  ...
  labs(x = "Year", y = "ghg",
  title = "Inbound GHG of Vehical Emissions in Redwood City, 2013 to 2019", 
color ="Fuel Type" ) + scale_linetype_manual(values = c("solid","dotted"), labels = c("LDT1", "LDA"))
```
...
Redwood_trips_ghg_full <-
Redwood_trips_ghg_full_temp %>%
  group_by(year,Category, Fuel_Type) %>%
summarise(
    ghg =sum(ghg, na.rm = T),
...
```{r}
ggplot(Redwood_pge_sum_zip, aes(x=CUSTOMERCLASS, y=TOTALKBTU))+
  geom_bar(stat='identity', fill="forest green")+
  facet_wrap(~ZIPCODE)
```
...
Redwood_pge_residential_19   %>% 
  ...
  addLayersControl(
    overlayGroups = c("kBtu", "TOTAL kBtu/resident"),
...
Redwood_pge_residential_19   %>% 
  ...
  addLayersControl(
    overlayGroups = c("tCO2e","TOTAL tCO2e/resident"),
    position = "bottomright",
    options = layersControlOptions(collapsed = FALSE)
  ) %>% 
  addControl("Residential Electricity", position = "bottomright") %>%
...
Redwood_pge_residential_19   %>% 
  ...
  addLayersControl(
    overlayGroups = c("tCO2e", "TOTALTCO2E/resident"),
    position = "bottomright",
    options = layersControlOptions(collapsed = FALSE)
  ) %>% 
...
Redwood_pge_commercial_19   %>% 
  ...
  addLayersControl(
    overlayGroups = c("kBtu", "TOTALKBTU/job"),
    position = "bottomright",
    options = layersControlOptions(collapsed = FALSE)
  ) %>% 
...
Redwood_pge_commercial_19   %>% 
  ...
  addLayersControl(
    overlayGroups = c("tCO2e", "TOTALTCO2E/job"),
    position = "bottomright",
    options = layersControlOptions(collapsed = FALSE)
  ) %>% 
  addControl("Commercial Electricity", position = "bottomright") %>%
...
Redwood_pge_commercial_19   %>% 
  ...
  addLayersControl(
    overlayGroups = c("kBtu", "TOTALTCO2E/job"),
    position = "bottomright",
    options = layersControlOptions(collapsed = FALSE)
  ) %>% 
  addControl("Commercial Gas", position = "bottomright") %>%
...
```{r}
Redwood_emissions_plot <-
  Redwood_emissions %>%
  group_by(Year,Emissions,Emission_Source) %>%
  summarise(
    Sum_emissions= sum(Emissions, na.rm = T)
  )
```
...
ggplot(
  ...
  geom_bar(stat = "identity", aes(fill =Emission_Source), position = "dodge") + 
  labs(x = "Year", y = "TOTALTCO2E/GHG", title ="Vehicle/Building Emissions in Redwood City,\n2013 to 2019") + 
  scale_fill_discrete(name="Emission_Source",labels = c("Building","Vehicle"))  
```
